...
public class Carrier extends Window implements Checkable {
    ...
    public Carrier() {
        super(new Coord(200,200), nurgling.translation.L10n.get("Carrier"));
        NCarrierProp startprop = NCarrierProp.get(NUtils.getUI().sessInfo);
        prev = add(new Label(nurgling.translation.L10n.get("Carrier Settings:")));
        prev = add(textEntry = new TextEntry(300,startprop.object == null ? "" : startprop.object), prev.pos("bl").add(UI.scale(2,5)));
        prev = add(new Button(UI.scale(150), nurgling.translation.L10n.get("Start")){
            @Override
            public void click() {
                super.click();
