...
public class CarrotFarmer implements Action {
    ...
    public Results run(NGameUI gui) throws InterruptedException {
        ...
        if(new Validator(req, opt).run(gui).IsSuccess())
        {
            ...
            if(NArea.findOut("Carrot", 1)!=null)
                new CollectItemsToPile(NArea.findSpec(field).getRCArea(),NArea.findOut("Carrot", 1).getRCArea(),new NAlias("items/carrot", "Carrot")).run(gui);
