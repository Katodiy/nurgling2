...
/**
 * Test bot that demonstrates the cheese navigation WITH the fix.
 *
 * Flow:
 * 1. Navigate to first cheese area (buffer containers)
 * 2. Find containers, store references in list (for comparison)
 * 3. Navigate to a DIFFERENT cheese area (simulates FreeInventory2)
 * 4. Navigate BACK to original area
 * 5. RE-FIND containers with FRESH references (THE FIX)
 * 6. Try to PathFinder to containers using fresh references
 *
 * Expected result: PathFinder succeeds because we use fresh gob references
 */
public class CheeseNavTestWithFix implements Action {

    @Override
    public Results run(NGameUI gui) throws InterruptedException {
        gui.msg("=== CheeseNavTest WITH FIX ===");
        gui.msg("This test demonstrates the fix: re-finding containers after navigation");
        ...
        // Step 2: Find containers and store references (for comparison only)
        ...
        gui.msg("Found " + originalContainers.size() + " containers (storing for comparison)");
        ...
        // Step 5: THE FIX - Re-find containers with FRESH references
        gui.msg("Step 5: RE-FINDING containers with FRESH references (THE FIX)...");
        ArrayList<Gob> freshContainers = Finder.findGobs(sourceArea,
            new NAlias(new ArrayList<>(NContext.contcaps.keySet()), new ArrayList<>()));

        gui.msg("Found " + freshContainers.size() + " containers with fresh references");

        // Compare old vs new references
        gui.msg("Comparing old vs fresh references:");
        for (int i = 0; i < Math.min(originalContainers.size(), freshContainers.size()); i++) {
            Gob oldGob = originalContainers.get(i);
            Gob freshGob = freshContainers.get(i);
            boolean sameObject = (oldGob == freshGob);
            boolean sameId = (oldGob.id == freshGob.id);
            gui.msg("  Container " + i + ": sameObject=" + sameObject + ", sameId=" + sameId);
        }

        // Step 6: Try to access containers using FRESH references (WITH FIX)
        gui.msg("Step 6: Testing PathFinder with FRESH container references (WITH FIX)...");

        int successCount = 0;
        int failCount = 0;

        for (Gob container : freshContainers) {
        ...
        // Report results
        gui.msg("=== TEST RESULTS (WITH FIX) ===");
        gui.msg("Total containers: " + freshContainers.size());
        gui.msg("PathFinder SUCCESS: " + successCount);
        gui.msg("PathFinder FAIL: " + failCount);

        if (failCount == 0) {
            gui.msg("FIX CONFIRMED: Fresh gob references work correctly");
        } else {
            gui.msg("Some failures detected (may be unrelated issues)");
        }
