...
public class NCuriosity extends Curiosity implements GItem.OverlayInfo<Tex>{
    ...
    public BufferedImage tipimg() {
        ...
        if(exp > 0)
            buf.append(String.format(nurgling.translation.L10n.get("Learning points:") + " $col[192,192,255]{%s} ($col[192,192,255]{%s}/h)\n", Utils.thformat(exp), Utils.thformat(Math.round(exp / (time / 3600.0)))));
        if(time > 0) {
            buf.append(String.format(nurgling.translation.L10n.get("Study time:") + " $col[192,255,192]{%s} ($col[192,255,255]{%s})\n", timefmt(time), timefmt((int)(time/server_ratio))));
        }
        rm = (int)(remaining()/server_ratio);
        if(rm!=time)
        {
            buf.append(String.format(nurgling.translation.L10n.get("Remaining time:") + " $col[192,255,192]{%s}\n", timefmt(rm)));
        }
        if(mw > 0)
            buf.append(String.format(nurgling.translation.L10n.get("Mental weight:") + " $col[255,192,255]{%d}\n", mw));
        if(enc > 0)
            buf.append(String.format(nurgling.translation.L10n.get("Experience cost:") + " $col[255,255,192]{%d}\n", enc));
        if(lph>0) {
